...
bool ClassTable :: has_main(Symbol m){
    return get_formals(m, main_meth) != NULL ;
}
...
bool ClassTable::is_valid() {
    ...
    for (std::map<Symbol, Symbol>::iterator iter = inheritance_graph.begin();
        ...
        // cout<<"Checking "<< child->get_string();
        // ostream& sr = semant_error(class_map[child]);
        // sr<<child->get_string()<<" "<< parent->get_string()<<endl;
        
        // check if Main  class is defined, and if main function is defined
        if (child == Main){
            is_main_defined = true;
            if (has_main(child)) has_main_fn = true;
        } 
        while (parent != No_class) {
            ...
            else if (inheritance_graph.find(parent) == inheritance_graph.end())
            {
                ...
                // cout<<"climb:" << child->get_string()<<" -> " << parent->get_string()<<endl;
...
void ClassTable::display(){
    typedef std::map<Symbol, Symbol> c;
    for(c::iterator i = inheritance_graph.begin(); i != inheritance_graph.end(); i++){
        cout <<  i->first->get_string() <<" -> "<< i->second->get_string() <<" ";
        if(inheritance_graph.find(i->second) != inheritance_graph.end()) cout<<"found";
        cout<<endl;
    }
}
...
// as method, noting to do when adding to environment table
 ...
 * 1. In a new environment scope, bind the keyword self. => specify SELF_TYPE
 * 2. Recursively check the formal parameters in this environment. => start check for expressions
...
Expression assign_class::type_check(type_env_t env) {
    ...
    // cout<< t1->get_string() << " <- "<< t2->get_string() << endl; 
    if (t2 == SELF_TYPE) {
        ostream& err_stream = env.ct->semant_error(env.curr->get_filename(), this);
...
Expression typcase_class::type_check(type_env_t env) {
    ...
    for (int i = cases->first(); cases->more(i); i = cases->next(i)) {
        if (i != cases->first()) {
            env.om->enterscope();
            tn = env.ct->lca(tn, cases->nth(i)->type_check(env));
            env.om->exitscope();
            // return type of Case will be the LCA of all branches
        }
    }
    type = tn;
    return this;
